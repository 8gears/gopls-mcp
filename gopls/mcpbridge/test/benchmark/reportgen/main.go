...
func main() {
	...
	fmt.Printf("✅ Generated %s from %s (compact=%v)\n", outputFile, jsonFile, *compact)
}
...
func generateReport(suite BenchmarkSuite, compact bool) string {
	...
	// Header
	sb.WriteString("# 🚀 gopls-mcp Benchmark Results\n\n")

	if !compact {
		sb.WriteString("⚠️ **AUTO-GENERATED** - This file is automatically generated from `benchmark_results.json`\n\n")
	...
	if compact {
		sb.WriteString("### ⚡ Comparison vs CLI\n\n")
	} else {
	...
	if len(comparisons) > 0 {
		if compact {
			...
			for _, r := range comparisons {
				...
				sb.WriteString(fmt.Sprintf("| **%s** | %d | %s | %s | %s | ±%s | %.1f%% | %s | **%.1fx** |\n",
	...
	if !compact {
		...
		sb.WriteString("CV = (StdDev / Mean) × 100%\n\n")
		sb.WriteString("| CV Range | Interpretation |\n")
		sb.WriteString("|-----------|----------------|\n")
		sb.WriteString("| < 10% | ✅ Very consistent, reliable |\n")
		sb.WriteString("| 10-25% | ⚠️ Moderate variance, acceptable |\n")
		sb.WriteString("| > 25% | ❌ High variance, results less reliable |\n")
		sb.WriteString("\n")
		...
		for _, r := range suite.Results {
			if r.Iterations > 0 && r.StdDev > 0 {
				...
				cv := calculateCV(r.StdDev, r.Duration)
				sb.WriteString(fmt.Sprintf("| %s | %d | ±%s | %.1f%% | %s |\n",
					strings.Replace(r.Name, "Comparison: ", "", 1),
					r.Iterations,
					formatDuration(r.StdDev),
					cv,
